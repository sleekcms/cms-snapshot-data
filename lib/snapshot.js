...
const toSchemaTree = schemas => {
  ...
  let flatTree = Object.values(schemas).map(m => {
    return {
      ...
      name: m.name,
      is_list: m.is_list,
      children: !m.is_list && m.fields.reduce((acc, f) => {
...
class Snap {
  ...
  formatValue(field, value) {
    if (this.mock) {
      if (field.multiple) return _.fill(Array(_.random(5)), null).map(t => getMockValue({ field, env: this.env }));else return getMockValue({ field, env: this.env });
    }
    let type = field.type;
    if (type === "singleline") {
      if (field.multiple) return getMultiple(value);else return getOne(value);
    }
    if (type === "markdown") {
      if (field.multiple) return getMultiple(value).map(v => marked.parse(v));else return marked.parse(getOne(value) || '');
    }
    if (type === "json") {
      if (field.multiple) return getMultiple(value).map(v => safeJsonParse(v));else return safeJsonParse(getOne(value));
    }
    if (type === 'number') {
      if (field.multiple) return getMultiple(value).map(v => Number(v));else return Number(getOne(value));
    }
    if (type === 'image') {
      if (field.multiple) return getMultiple(value).map(v => this.getImageData(v, field));else return this.getImageData(getOne(value), field);
    }
    if (type === 'boolean') {
      if (field.multiple) return getMultiple(value);else return getOne(value);
    }
